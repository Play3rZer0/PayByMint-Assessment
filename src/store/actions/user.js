...
export const loadWeb3Modal = (web3Modal) => ({
  type: LOAD_WEB3MODAL,
  web3Modal,
});
...
export const onCorrectChain = (correctChain) => ({
  type: ON_CORRECT_CHAIN,
  correctChain,
});

export const onProvider = (payload) => ({ type: ON_PROVIDER, payload });

export const onBasicAccountData = (payload) => ({
  type: ON_BASIC_ACCOUNT_DATA,
  payload,
});

export const connectingWallet = (wallet) => ({
...
export const setShowWrongChainModal = (status) => ({
  type: SET_SHOW_WRONG_CHAIN_MODAL,
  status,
});

export const updateBlockchain = (blockchain) => ({
  ...
  async (dispatch) => {
    ...
    try {
      ...
      if (!!blockchain) {
        _chainId =
          blockchain === "Ethereum"
            ? ETHEREUM_SETTING.CHAINID
            : BINANCE_SETTING.CHAINID;
      } else _chainId = ETHEREUM_SETTING.CHAINID;

      const correctChain =
        cid === _chainId || parseInt(cid) === Number(_chainId);
...
export const chainConnect = (_chainId) => async (dispatch) => {
  const WEB3_CONNECT_CACHED_PROVIDER = localStorage.getItem(
    "WEB3_CONNECT_CACHED_PROVIDER"
  );
